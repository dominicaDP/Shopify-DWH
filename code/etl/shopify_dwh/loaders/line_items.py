"""
stg_order_line_items loader.

Line items are children of orders, extracted from the orders query's nested
lineItems connection. The revenue-bearing table for the headline metric.

Modes mirror the orders loader:
...
Known limitation (carried from the POC, flagged in build-plan.md §5 for hardening):
the inner connection is fetched at lineItems(first: %d); an order with more lines
would be truncated. We log a warning if that ever happens — rare for DYT, but a
production hardening step should page the inner connection or use Bulk Operations.
Loads into <stg_schema>.stg_order_line_items.

Run (from code/etl/):
    python -m shopify_dwh.loaders.line_items            # auto: full if empty, else incremental
    python -m shopify_dwh.loaders.line_items full
    python -m shopify_dwh.loaders.line_items incremental
"""
...
def _print_summary(conn, schema: str, processed: int, mode: str) -> None:
    stats = conn.execute(
...
def main() -> int:
    ...
    try:
        ...
        with ShopifyClient.from_settings(settings) as client:
            if mode == "full":
                processed = run_full(conn, client, schema)
            elif mode == "incremental":
                processed = run_incremental(conn, client, schema)
            else:
                print(f"unknown mode: {mode!r} (use full | incremental)", file=sys.stderr)
                return 2
        _print_summary(conn, schema, processed, mode)
    finally:
        conn.close()
    return 0
