...
Ported verbatim from the POC (only the constructor entry points changed to take a
Settings object). It handles the things every loader needs:
...
Self-test (paginates a few products):
    python -m shopify_dwh.shopify_client
"""
...
class ShopifyClient:
    ...
    @classmethod
    def from_settings(cls, settings: Settings, **kwargs) -> "ShopifyClient":
        s = settings.shopify
        return cls(
    ...
    def from_env(cls, **kwargs) -> "ShopifyClient":
        return cls.from_settings(load_settings(), **kwargs)
...
# ---------------------------------------------------------------------------
# Self-test: prove auth + pagination + cost reporting work end to end.
# ---------------------------------------------------------------------------
