-- =============================================================================
-- Phase C — generate dim_date (2020-01-01 .. 2030-12-31, 4018 days)
-- Run AFTER 02_dwh_schema.sql.  TRUNCATE + INSERT, so re-running is idempotent.
--
-- Exasol has no generate_series: build numbers 0..9999 from a 4-digit cross join,
-- add to a start date, keep the 11-year span (ported from the POC, validated).
--
-- day_of_week is 1 = Monday .. 7 = Sunday (design convention; the POC used 0..6).
-- Anchored on 2024-01-01 (a Monday). The +700000 (a multiple of 7) keeps MOD
-- non-negative for pre-anchor dates;  MOD(...) is 0..6 for Mon..Sun, +1 -> 1..7.
-- fiscal_* default to the calendar year/quarter — no fiscal offset is set for DYT;
-- change the two fiscal expressions here if a non-January fiscal start is adopted.
-- =============================================================================

TRUNCATE TABLE SHOPIFY_DWH.dim_date;

INSERT INTO SHOPIFY_DWH.dim_date
    (date_key, full_date, cal_year, cal_quarter, cal_month, month_name,
     week_of_year, day_of_month, day_of_week, day_name, is_weekend,
     is_month_end, fiscal_year, fiscal_quarter)
WITH digits AS (
...
SELECT
    ...
    MOD(DAYS_BETWEEN(full_date, DATE '2024-01-01') + 700000, 7) + 1 AS day_of_week,
    ...
    CAST(CEIL(MONTH(full_date) / 3.0) AS INTEGER) AS fiscal_quarter
FROM cal;
