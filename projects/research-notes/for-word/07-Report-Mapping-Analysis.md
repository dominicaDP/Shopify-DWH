...
|--------|---|-----------------|--------|
| DYT Revenue by Client (In Process) | 17 | "In Process" lifecycle status | RESOLVED - "In Process" = pending refunds. Filter on dim_voucher |
| DYT Missing Redemption Info | 20 | Data quality / exception tracking | Still needed - exception reporting pattern |
| STD Bank Membership Order Summary | 37 | Membership tiers and discount levels | PARTIALLY RESOLVED - tiers likely in Shopify discount code names |
| DYT Promo Pricing | 32 | Promotional pricing dimension | Still needed - may use Shopify compare_at_price |
...
- Pattern B: Campaign = "Refund" + Client = reason ("Damaged", "Missing items", etc.)
- Both patterns exist - needs normalisation in ETL

**Schema impact:**

- **Client maps to dim_channel** (the clean business entity dimension)
- **Campaign maps to campaign_name VARCHAR on dim_voucher** (too varied for a standalone dimension, but essential for filtering/grouping)
...
**Schema impact:** Rename additional_spend to overspend in DYT_DWH tables for business alignment.
...
| Overspend | Report 21 | RESOLVED - customer spend beyond voucher | Rename additional_spend to overspend in DYT_DWH |
...
2. **Update Layer 2 schema** (schema-dyt.md) with confirmed findings:
   ...
   - Rename additional_spend to overspend
